pipeline {
    ...
    stages {
        ...
        stage("Register Task Definition") {
            steps {
                sh "eval `/var/jenkins_home/.local/bin/aws ecs register-task-definition --family flask-signup --cli-input-json file://common-task-definition-v_${params.build_tag}.json`"
                ...
                sh "eval `/var/jenkins_home/.local/bin/aws ecs update-service --cluster default --service ${SERVICE_NAME} --task-definition ${env.TASK_FAMILY}:${params.build_tag}`"
            }
